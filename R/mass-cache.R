#' Populate the API cache
#'
#' Use this path when you have lots of query to run and (>1000)
#' in order to resolve all of them at once without caring about the results
#' (except errors). See Details for how to specify parameter values
#'
...
#'
#' @note If using your own server do not add the `/mass_cache` at the end of the
...
mass_cache <- function(ips,
  ...
  mass_cache_endpoint <- file.path(ip_asn_history_endpoint, "/mass_cache")
  mass_cache_endpoint <- httr::parse_url(mass_cache_endpoint)
  mass_cache_endpoint$path <- gsub("[/]{2,}", "/", mass_cache_endpoint$path)
  mass_cache_endpoint <- httr::build_url(mass_cache_endpoint)

  httr::POST(
    url = mass_cache_endpoint,
    encode = "json",
    body = dict,
    .IPASNHISTORY_UA
  ) -> res
